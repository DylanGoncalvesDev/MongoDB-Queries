...
db.restaurante.find({},{restaurant_id: 1, name: 1})
...
db.restaurante.find({},{restaurant_id: 1, name: 1, borough: 1, "address.zipcode": 1})
...
db.restaurante.aggregate([{$addFields: {total_score: { $sum: "$grades.score" }}},{$match: {total_score: { $gt: 90 }}}])
// 9. Trobar els restaurants amb un score de més de 80 però menys que 100.
db.restaurante.aggregate([{$addFields: {total_score: { $sum: "$grades.score" }}},{$match: { total_score: { $gt: 80, $lt: 100 }}}])
// 10. Trobar els restaurants amb longitud menor que -95.754168.
db.restaurante.find({"location.coordinates.0": {$lt: -95.754168}})
// 11. Trobar restaurants que no preparen 'American', amb qualificació > 70 i longitud < -65.754168.
db.restaurante.aggregate([{ $addFields: {total_score: { $sum: "$grades.score"}}}, {$match: {cuisine: { $ne: "American"},total_score: { $gt: 70},"location.coordinates.0": {$lt: -65.754168}}}])
// 12. El mateix que l'anterior però sense usar operador $and.
db.restaurante.aggregate([{$addFields: { total_score: { $sum: "$grades.score"}}},{$match: {$and: [{cuisine: { $ne: "American"}},{total_score: { $gt: 70}},{"location.coordinates.0": {$lt: -65.754168}}]}}])
// 13. Trobar restaurants que no són 'American', grau 'A', i no són de Brooklyn. Ordenats per cuisine descendent.
db.restaurante.aggregate([ {$match: {cuisine: { $ne: "American"},grades: {$not: {$elemMatch: {grade: "A"}}}, borough: {$ne: "Brooklyn"}}},{$sort: {cuisine:-1}}])
...
db.restaurante.find({$or: [{cuisine: "Seafood"},{name: /^Wil/}]})
...
db.restaurante.find({location: {$near: {geometry: {type: "Point",coordinates: [-74, 40.7]}, $maxDistance: 5000}}})
...
db.restaurante.find({"location.coordinates.0": {$type: "double"},"location.coordinates.1": {$type: "double"}},{name: 1, restaurant_id: 1, "location.coordinates": 1})
...
db.restaurante.find({name: /mon/},{name: 1, borough: 1, "location.coordinates": 1, cuisine: 1})
// 32. Mostrar restaurant_id, name i grade i score de més de 80 però menys que 100.
db.restaurante.find({"grades.score": {$gt: 80, $lt: 100}},{restaurant_id: 1, name: 1, grades: 1})

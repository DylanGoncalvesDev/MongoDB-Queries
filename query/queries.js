...
db.restaurants.aggregate([{ $addFields: { total_score: { $sum: "$grades.score" }}},{ $match: { total_score: { $gt: 90 }}}])
// 9. Trobar els restaurants amb un score de més de 80 però menys que 100.
db.restaurants.aggregate([{ $addFields: { total_score: { $sum: "$grades.score" }}},{ $match: { total_score: { $gt: 80, $lt: 100 }}}])
...
db.restaurants.aggregate([{ $addFields: { total_score: { $sum: "$grades.score"}}}, { $match: { cuisine: { $ne: "American"}, total_score: { $gt: 70 },"location.coordinates.0": { $lt: -65.754168 }}}])
// 12. El mateix que l'anterior però sense usar operador $and.
db.restaurants.aggregate([{ $addFields: { total_score: { $sum: "$grades.score" }}},{ $match: { $and: [{ cuisine: { $ne: "American" }},{ total_score: { $gt: 70 }},{ "location.coordinates.0": { $lt: -65.754168 }}]}}])
// 13. Trobar restaurants que no són 'American', grau 'A', i no són de Brooklyn. Ordenats per cuisine descendent.
db.restaurants.aggregate([ { $match: { cuisine: { $ne: "American" }, grades: { $not: { $elemMatch: { grade: "A" }}}, borough: { $ne: "Brooklyn" }}},{ $sort: { cuisine:-1 }}])
...
db.restaurants.find({ "location.coordinates.0": { $type: "double" }, "location.coordinates.1": { $type: "double" }},{ name: 1, restaurant_id: 1, "location.coordinates": 1 })
...
db.restaurants.find({ name: /mon/ },{ name: 1, borough: 1, "location.coordinates": 1, cuisine: 1 })
// 32. Mostrar restaurant_id, name i grade i score de més de 80 però menys que 100.
db.restaurants.find({ "grades.score": { $gt: 80, $lt: 100 }},{ restaurant_id: 1, name: 1, grades: 1 })
